...
plot(data$baranina)
plot(data$chicken)
#Исходя из визуального анализа можно увидеть что времянной ряд имеет нисходящий тренд и сезонность -> ряд нестационарный
#cor(bbroad$`t=time`, bbroad$butterbroad,method = "spearman")
#cor(bbroad$`t=time`, bbroad$butterbroad,method = "kendall")


#тест дики фуллера на стационарность времянного ряда 
library(tseries)
...
# График автокорреляционной функции для 
acf(data$baranina)
acf(data$chicken)
